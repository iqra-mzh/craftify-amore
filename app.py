...
# --- G: DRIVE PATH SETUP ---
# This ensures all operations stay on your G: drive
BASE_DIR = r"G:\Craftify_Studio"
...
st.markdown("""
    ...
    .stButton>button { width: 100%; border-radius: 20px; background-color: #ffb7c5; color: white; }
...
# --- 3. TINYWOW SUITE ---
    ...
    if st.button("Process & Download"):
        if files:
            # Here is where you would process files. 
            # For now, it simulates a save to G: drive
            st.balloons()
...
# --- 4. VIDEO CENTER
    ...
    urls = st.text_area("Paste Video URLs (one per line):", placeholder="https://youtube.com/...")
    quality = st.selectbox("Select Quality", ["720p", "1080p", "4K", "MP3 Audio Only"])
    if st.button("Start Bulk Download"):
        if urls:
            st.write(f"Initiating downloads to G: drive...")
